...
struct mdt {
  ...
  mdt()
      : cl("MDT"), kb_in(cl, "Kb IN", 1), kb_out(cl, "Kb OUT", 0),
        synth_out(cl, "Synth OUT", 0){};

  bool connect_ports_kb(const char *pattern, size_t idx, const char *a,
                        bool invert) {
  ...
  void connect_ports() {
    if (kb_out.num() == 0)
      connect_ports_kb("system:midi_playback_[0-9]+", 1, kb_out.name(), 0);
    if (kb_in.num() == 0)
      connect_ports_kb("system:midi_capture_[0-9]+", 2, kb_in.name(), 1);
    if (kb_in.num() && kb_out.num())
      std::cerr << "Keyboard connected" << '\n';

    if (synth_out.num() == 0) {
      bool b = connect_ports_synth();
      if (b)
        std::cerr << "Synth connected" << '\n';
    };
  };

  enum subev_t { SUBEV_REORDER; };
  struct event {
    size_t abstime;
    std::variant<jack::midi_event, std::function<void(mdt &)>, subev_t> data;

    template <class A> event(size_t _t, const A &_d) : abstime(_t), data(_d){};
  };
  struct event_cmp {
  ...
  template <class Fun> void set_timer(size_t delay, Fun f) {
    event_queue.emplace(frames + delay, f);
  };
  template <class Fun> void set_timer_repeat(size_t delay, Fun f) {
  ...
  struct processor {
    ...
    jack_nframes_t localtime() { return m.frames - timebase; };

    processor(mdt &_m, size_t tb, jack_nframes_t nf)
        : m(_m), timebase(tb), kbwbuf(m.kb_out, nf), snwbuf(m.synth_out, nf){};
  ...
  void process(jack_nframes_t nf) {
    ...
    while (!event_queue.empty()) {
      ...
      if (ev.abstime >= (timebase + nf))
        break;
      frames = ev.abstime;

      std::visit(
          overloaded{[&](const jack::midi_event &me) {
                       prc.process_midi(ev.abstime, me);
                     },
                     [&](const std::function<void(mdt &)> &fun) { fun(*this); },
                     [&](const subev_t t) { if (t == SUBEV_REORDER) }},
          ev.data);
      ...
      if (update_ledbuf) {
        for (uint8_t i = 0; i < 16; i++) {
          if (ledbuf[i] != ledbuf_old[i]) {
            prc.kbwbuf.write(jack::midi_event(
                prc.localtime(), {159, (uint8_t)(36 + i), ledbuf[i]}));
